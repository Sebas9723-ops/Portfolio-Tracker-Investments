...
Uses yfinance .financials / .balance_sheet / .cashflow for statements.
NO yf.Ticker().info calls — uses fast_info + direct statement DataFrames.
"""
...
def _fetch_financials(ticker: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Returns (income_stmt, balance_sheet, cashflow) DataFrames."""
    import yfinance as yf
    t = yf.Ticker(ticker)
    try:
        inc = t.financials
...
def _fetch_valuation_metrics(ticker: str) -> dict:
    """
    Fetch valuation metrics. Uses yf.Ticker.info wrapped in try/except.
    Falls back to derived values from fast_info + financials where possible.
    """
    import yfinance as yf
    metrics = {
