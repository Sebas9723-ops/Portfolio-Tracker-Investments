...
def _build_chart(df: pd.DataFrame, ticker: str) -> go.Figure:
    fig = make_subplots(
        ...
        subplot_titles=[f"{ticker} · Price & Indicators", "RSI (14)", "MACD (12/26/9)"],
    )
    ...
    fig.add_trace(go.Scatter(
        ...
        fill="toself", fillcolor="rgba(100,149,237,0.07)",
        line=dict(color="rgba(0,0,0,0)"), name="Bollinger", hoverinfo="skip",
    ), row=1, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["BB_Upper"], line=dict(color="#6495ed", width=1, dash="dot"), name="BB Upper", hovertemplate="%{y:.2f}<extra>BB Upper</extra>"), row=1, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["BB_Lower"], line=dict(color="#6495ed", width=1, dash="dot"), name="BB Lower", hovertemplate="%{y:.2f}<extra>BB Lower</extra>"), row=1, col=1)

    # Candlestick
    fig.add_trace(go.Candlestick(
        x=df.index, open=df["Open"], high=df["High"], low=df["Low"], close=df["Close"],
        increasing_line_color="#00e676", decreasing_line_color="#f44336",
        name="Price",
    ), row=1, col=1)

    # Moving averages
    for col_name, color, label in [("SMA20", "#f3a712", "SMA 20"), ("SMA50", "#00c8ff", "SMA 50"), ("SMA200", "#ce93d8", "SMA 200")]:
        s = df[col_name].dropna()
        if not s.empty:
            fig.add_trace(go.Scatter(x=df.index, y=df[col_name], line=dict(color=color, width=1.5), name=label, hovertemplate=f"%{{y:.2f}}<extra>{label}</extra>"), row=1, col=1)

    # RSI
    fig.add_trace(go.Scatter(x=df.index, y=df["RSI"], line=dict(color="#f3a712", width=1.5), name="RSI", hovertemplate="%{y:.1f}<extra>RSI</extra>"), row=2, col=1)
    fig.add_hline(y=70, line_dash="dot", line_color="#f44336", line_width=1, row=2, col=1)
    fig.add_hline(y=30, line_dash="dot", line_color="#00e676", line_width=1, row=2, col=1)
    fig.add_hrect(y0=70, y1=100, fillcolor="rgba(244,67,54,0.05)", line_width=0, row=2, col=1)
    fig.add_hrect(y0=0, y1=30, fillcolor="rgba(0,230,118,0.05)", line_width=0, row=2, col=1)

    # MACD
    hist_colors = ["#00e676" if v >= 0 else "#f44336" for v in df["MACD_Hist"].fillna(0)]
    fig.add_trace(go.Bar(x=df.index, y=df["MACD_Hist"], marker_color=hist_colors, name="Histogram", hovertemplate="%{y:.4f}<extra>Hist</extra>"), row=3, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["MACD"], line=dict(color="#00c8ff", width=1.5), name="MACD", hovertemplate="%{y:.4f}<extra>MACD</extra>"), row=3, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df["MACD_Signal"], line=dict(color="#f3a712", width=1.5), name="Signal", hovertemplate="%{y:.4f}<extra>Signal</extra>"), row=3, col=1)

    fig.update_layout(
        paper_bgcolor="#0b0f14", plot_bgcolor="#0b0f14",
        font=dict(color="#e6e6e6", size=12), height=740,
...
def render_technicals_page(ctx):
    ...
    with c2:
        if use_custom:
            ticker = st.text_input("Ticker", placeholder="e.g. NVDA", label_visibility="collapsed", key="ta_custom_input").upper().strip()
        else:
            ticker = st.selectbox("Select ticker", portfolio_tickers, key="ta_portfolio_sel") if portfolio_tickers else ""
    with c3:
    ...
    st.markdown("")
    info_section("Chart", f"{ticker} · {period} · Candlestick · SMA 20/50/200 · Bollinger Bands · RSI · MACD")
