...
Connects to XTB via WebSocket (xAPI protocol).
Handles positions, trade history, live prices, and order execution.
...
# ── Symbol mapping ─────────────────────────────────────────────────────────────
# App ticker → list of possible XTB symbol names (tried in order)
...
# XTB symbol → app ticker (built from above, first match wins)
...
class XTBClient:
    """Synchronous XTB xStation API client over WebSocket."""

    def __init__(self, account_id: str, password: str, mode: str = "real"):
        self._account_id = str(account_id)
        self._password = password
        self._endpoint = _ENDPOINTS.get(mode.lower(), _ENDPOINTS["real"])
        self._ws = None
    ...
    def get_trades_history(self, start_ms: int = 0) -> list[dict]:
        """Closed trade history since start_ms (epoch milliseconds)."""
    ...
    def get_tick_prices(self, symbols: list[str]) -> dict[str, dict]:
        """Latest bid/ask for a list of XTB symbols."""
        ...
        if not resp.get("status"):
            return {}
        quotations = resp.get("returnData", {}).get("quotations", [])
        return {q["symbol"]: q for q in quotations}

    def get_symbol_info(self, symbol: str) -> dict:
        resp = self._send("getSymbol", {"symbol": symbol})
        return resp.get("returnData", {}) if resp.get("status") else {}
    ...
    def place_order(
        ...
        volume: float,     # shares
        comment: str = "PortafolioManagementSA",
    ) -> dict:
        """
        Place a market order.
        Returns {"order": order_id} on success or {"error": ...} on failure.
        """
    ...
    def __enter__(self):
        self.connect()
        if not self.login():
            raise RuntimeError("XTB login failed — check credentials.")
        return self
...
def load_xtb_positions() -> tuple[list[dict], str]:
    """Returns (open_trades, error_message). Cached 60 s."""
...
def load_xtb_account() -> tuple[dict, str]:
    """Returns (account_info, error_message). Cached 5 min."""
    account_id, password, mode = _get_xtb_cfg()
    if not account_id or not password:
        return {}, "XTB no configurado."
    try:
...
def load_xtb_history(start_year: int = 2023) -> tuple[list[dict], str]:
    """Returns (history_trades, error_message). Cached 1 h."""
    import calendar
    account_id, password, mode = _get_xtb_cfg()
    if not account_id or not password:
        return [], "XTB no configurado."
...
def trades_to_shares(trades: list[dict]) -> dict[str, float]:
    ...
    Aggregates multiple positions in the same instrument.
    """
...
def history_to_transactions(history: list[dict]) -> pd.DataFrame:
    """
    Convert XTB trade history to a DataFrame matching TRANSACTIONS_HEADERS:
    [date, ticker, type, shares, price, notes]

    XTB history returns CLOSED positions. Each record contains:
      - open_time  / open_price  → BUY transaction
      - close_time / close_price → SELL transaction (only if close_price > 0)
    """
    ...
    for t in history:
        ...
        # BUY leg (cmd=0 means long, opened with a buy)
        if cmd == 0:
...
def build_positions_comparison(
    xtb_shares: dict[str, float],
    app_df: pd.DataFrame,
) -> pd.DataFrame:
    """
    Compare XTB shares vs app-tracked shares.
    Returns a DataFrame with columns: Ticker, XTB Shares, App Shares, Δ Shares, In Sync
    """
    tickers = sorted(set(xtb_shares) | set(app_df["Ticker"].tolist() if not app_df.empty else []))
    app_map = (
        app_df.set_index("Ticker")["Shares"].to_dict() if not app_df.empty else {}
    )
    rows = []
    for t in tickers:
